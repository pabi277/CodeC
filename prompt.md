...
- **Phase 45** (the guide: five slides on first run + a ten-beat tour over the real controls) — 🚧 IMPLEMENTED through **round 2** on `arena/01a0955a-codec` (both parts; CI ✅ GREEN round 1 `34698914219` on tip `3c597b2`, release APK +0.19%; round 2 rebuilt 45.2 after the owner's device report — ten ordered beats, no next button, tap-outside inert, measured card height, no box behind a dialog or a closed drawer, `demo_flask` always present — **175 host cases green locally, CI ✅ GREEN round 2 `34704379023`**, release APK +96 B over round 1); **device round NOT run** — `docs/chat-phase45/DEVICE_ROUND.md` (G1-G14) — `docs/chat-phase45/`. The copy is vocabulary-pinned to the real tree (`GuideVocabulary` + `GuideTermProof`: a word that is not a real button label, file name or language fails the build), the anchors withdraw on dispose so a mark can never point at a control that is not on screen, and the two new Settings rows are audit rows **53-54**.
