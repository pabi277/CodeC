...
- **Phase 45** (the guide: five slides on first run + a ten-beat tour that cannot be skipped) — 🚧 IMPLEMENTED through **round 3** on `arena/01a0955a-codec` (both parts; CI ✅ GREEN round 1 `34698914219` and round 2 `34704379023`, release APK +96 B over round 1; round 2 made 45.2 one ordered tour, round 3 removed its skip after the owner ran it — every beat waits, a tour card has no button at all, Back pauses without spending, the finish card offers VIEW AGAIN + CLOSE, beat 2 is anchored in every project state, beat 6 on the bar as well as the handle, the drawer reopens after a project pick, and a route-aware 20 s stall guard is the only thing that can pass a beat — **180 host cases green locally, CI ✅ GREEN round 3 `34707337429`**, release APK +0.08% over round 2); **device round NOT run** — `docs/chat-phase45/DEVICE_ROUND.md` (G1-G14) — `docs/chat-phase45/`. The copy is vocabulary-pinned to the real tree (`GuideVocabulary` + `GuideTermProof`: a word that is not a real button label, file name or language fails the build), the anchors withdraw on dispose so a mark can never point at a control that is not on screen, and the two new Settings rows are audit rows **53-54**.
