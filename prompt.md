...
**PHASE 45 IS 🚧 IMPLEMENTED THROUGH ROUND 5 — 45.2 IS ONE TEN-BEAT TOUR THAT CANNOT BE SKIPPED AND, SINCE ROUND 4, WHERE ONE TAP DOES BOTH HALVES (round 2 rebuilt it as a tour; round 3 removed the skip after the owner ran it; round 4 made the highlighted control perform its own click because *"1st click disappear the massage and i have to click 2nd time to really work"*, and added the chrome lock because *"when the userland is installing and unpacking the user can not access any other option and it will show a sweet massage of why"*); CI ✅ GREEN ON ROUNDS 1-4 (`34698914219` tip `3c597b2`; `34704379023` tip `acadaee`; `34707337429` tip `0fcb3b6`; `34711827176` tip `e7759f1`, job `build` 11m29s, zero annotations, release APK 6,675,258 B = +5,400 B / +0.08% over round 3 and +23,576 B / +0.35% for the whole phase; `34714305062` tip `6c3cfea`, job `build` 25 steps 10m47s, zero annotations, release APK 6,675,254 B = −4 B against round 4 because one enum constant, a reordered `when` and one extra parameter are below what R8 can measure); 197 HOST CASES GREEN LOCALLY; DEVICE ROUND G1-G40 NOT RUN (2026-09-12, `arena/01a0955a-codec`; owner: "Start Phase 45" → four device reports → the tour rebuilt three times, the chrome lock twice).** The owner's row 2 — *"It has 0 guide features to give the user a real knowledge how to use the app, user don't know where should they change the project or file and the tap to the open down side of the keyboard"* → *"Set a step by step user guide after opening the app 1st time with a open view again[ing]"*, clarified as **"both layers"** — is now code, in the house shape (pure plan + thin Android edge, host-tested, no new dependency/permission/route). **45.1 (slides):** `ui/guide/GuidePlan.kt` — five `GuideSlide`s in the order the user meets the features (☰/files → RUN ▶ → the one-time download → the Terminal tab → Projects vs single files), caps title 34 / body 22 words / 130 chars, `at/canSkip/next/isLast/isDone/resume/progress/wordCount`, and **`GuideVocabulary`**: `candidateTerms` extracts what a slide *names* (control symbols ☰ ▶ ⋮, ALL-CAPS words, capitalised words that are not their sentence's first word, `backticked` spans) and every extracted term needs a `GuideTermProof(term, path, needle)` that `GuidePlanTest` checks against the REAL repo tree (`RepoFiles`), with three `commandProofs` (`pkg install`, `git`, `cc`) pinned in both directions — so the guide cannot promise a control that moved. `ui/guide/GuideScreen.kt` renders it (plain `Column`, SKIP on *every* slide, a fifth-per-slide progress bar, GOT IT → START CODING, back = SKIP, index surviving rotation/process death via `rememberSaveable` + `resume`) as the **second** first-launch gate — tiles → guide → shell — decided **before** Phase 44's `setupLaunchDivert` and the `NavHost`, so it can never share a screen with the setup bar and slide 3 explains the download the terminal is about to show. `guide_completed` (default **false**, so an upgrader sees it exactly once) is written in ONE place (the guide's `onFinished`, which SKIP/START CODING/back all call) and read once at startup, so Settings → **Reset tips** affects the *next* launch instead of yanking the user out of Settings. The owner's *"open view again[ing]"* is three doors on one local flag: Settings → About → **Help & guide** (audit row 53), Projects hub ⋮ → **Guide** (both hub states), editor ☰ drawer footer → **Guide**; it is **not** a nav route (a route would put the guide in the back stack and hand it to Phase 49's `BackRouter`), and the `NavController` is remembered above the gate so closing it lands back on the same tab. Safe mode skips the gate **and does not write the flag**. **45.2 (coach marks):** `ui/guide/CoachMarkPlan.kt` — five `CoachStep`s over `GuideSurface {EDITOR, TERMINAL, PACKAGES}` keyed by `GuideAnchors` (editor ☰, RUN ▶, the "Show tabs" handle, the Packages first card, the terminal status chip), `ChromeState.of(visibleAnchors, blockedByForeground)`, `stepsFor/nextUnseen/canShow/markSeen/parseSeen/serializeSeen/surfaceForRoute/stepForArrival`, `MAX_PER_ARRIVAL = 2`, plus `TooltipPlacement.place` over pure `GuideRect`/`GuideSize`. `ui/guide/CoachMarks.kt` is the edge: `GuideAnchorRegistry` (process-wide snapshot-state bridge, the codebase's `SetupNoticeBridge`/`EditorChromeState` idiom), `GuideAnchor.modifier(id)` publishing `boundsInWindow()` **and withdrawing the id in `DisposableEffect.onDispose`**, `GuideCoachMarks` (chrome → plan → per-arrival counter, reset on destination change), `CoachMarkOverlay` (scrim with a hole drawn as **four rects + a rounded stroke** — no `BlendMode.Clear`, so no offscreen layer — and a card placed by the pure function with an *estimated* 150 dp height). **The law that makes it safe:** `nextUnseen` only returns a step whose anchor the caller reports visible, and an all-hidden surface yields no step *and* no `seen` entry — so the "Show tabs" mark (the owner's *"the tap to the open down side of the keyboard"*) can never point at a handle that is not there, and it lands on the first arrival where the keyboard really hid the bar. **Never a trap:** the `Canvas` takes no pointer input (the control under the hole keeps working), and one `awaitFirstDown(requireUnconsumed = false)` consumes only OUTSIDE the hole, so tapping the highlighted control performs its own action *and* closes the mark; GOT IT and `BackHandler` are the other exits. The overlay is composed in a root `Box` **above** the `Scaffold` because the handle lives in `bottomBar`. Marks are blocked by the exit survey, safe mode, and a Phase 44 stage that is *actually moving* (DOWNLOADING/VERIFYING/EXTRACTING — **not** CHECKING, the tracker's startup value, or no mark would ever appear on a fresh phone); a screen's own dialogs/sheets are separate windows that cover the overlay and do not consume the pending step. `coach_marks_seen_csv` persists ids (garbage/deleted ids drop out in `parseSeen`); **Reset tips** (audit row 54) clears it *and* the guide flag in one atomic `dataStore.edit` touching no third key. **The vocabulary pin caught the plan's own copy:** slide 5 was specced *"⋮ → Open in editor"* but the hub's real label is `hub_open_action` = **"Open"**, so the shipped slide says *"Tap a card, or its ⋮ → Open, for the whole project."* **50 new host cases** (`GuidePlanTest` 15 · `CoachMarkPlanTest` 12 · `TooltipPlacementTest` 10 · `GuideWiringTest` 13 source pins) = **159 green locally** on the `rule.md` §9 kotlinc harness (one harness finding: a fixed 400-char window around `resetGuideTips` counted the *next* setter's key, so the pin slices to the function's own closing brace); the Compose edges cannot compile in the sandbox at all, so **CI's `Build APK` is the executor of record** and the exit condition is a **device** condition — [`docs/chat-phase45/DEVICE_ROUND.md`](docs/chat-phase45/DEVICE_ROUND.md) G1-G14 written and **NOT run; never describe Phase 45 as tested**. `SettingsAuditTest` stays green by construction (rows 53-54 in `docs/chat-phase38/SETTINGS_AUDIT.md` in the same commit: About 20 controls, 62 total, all `keep`) and `SettingsKeysHaveReadersTest` has real out-of-store readers. **ROUND 2 (the owner ran round 1's build):** *"the guided box are not consistent with flow … Not showing the full box guide at one and you didn't add all … remove the next option only the guide will show click the option where showing the guide to the next … make it like demo_flask is always present"*, and the flow he dictated: *"☰ bar → change the project folder to demo_flask → selected app.py → run → install → python → it will open the flusk web → close → tap to reveal the keyboard below option → then a small tour of package and terminal"*. **All four causes were in round 1's own code** (TROUBLESHOOTING §36): a fresh install blocked every box during Phase 44's download; `MAX_PER_ARRIVAL = 2` + the surface filter hid most beats; the card was placed with a **150dp estimated height** and clamped over its own hole; and a box could be cut **behind a dialog or a closed drawer**. **Now:** ten ordered beats (five new anchors — the drawer's project header, the drawer's `app.py` row, the preview's Back, the bottom bar's Packages + Terminal tabs; the five round-1 ids unchanged, so an upgrade keeps what it saw) under three pure laws: order, `waits` (the four always-there controls stop the tour until they are on screen — which is also the start gate, so the tour can only begin at the editor's ☰), and pass-over-**without**-spending; plus `EditorChromeState.dialogOpen`/`drawerOpen` above the anchor check. **The highlighted control is the only way forward:** no NEXT/GOT IT, outside taps swallow the whole gesture and do nothing (*"even tap outside will not end that box"*), **SKIP TOUR**/back call `markAllSeen` (one tap ends the tour, Reset tips is the door back), every card reads `Tour · n of 10`, and its height is **measured** (`onSizeChanged`). The slides are untouched by the owner's own decision (*"Slides stay as they are"*). **`demo_flask` is ALWAYS present** (re-seeded when missing, never overwritten — a recorded reversal of Phase 14's one-time law) because beats 2-3 teach it by name. **Recorded limit:** no box can point into an `AlertDialog`, so the project picker and the *Install Python?* prompt are taught by the copy of the beat before them (*"choose demo_flask"*, *"tap **Install**"*). **Delegated decision** (*"Don't understand you do as you like"*): Phase 44.1's terminal-first divert stays, so a fresh install still watches the download before the tour starts. **175 host cases green locally and CI ✅ GREEN on the round-2 commit (`34704379023`, tip `acadaee`, release APK +96 B over round 1)** (`CoachMarkPlanTest` 18, `GuideWiringTest` 16, `DemoProjectSeedTest` 7 — newly in the harness — `GuidePlanTest` 15, `TooltipPlacementTest` 10), and the round caught two faults before CI: a host call still passing the deleted `arrivalKey` argument, and a pin grepping the bare word "GOT IT" that failed on the comment promising there is none (**pin the button, never the word**). **The device round must start from Settings → About → Reset tips**, or round 1's seen beats hide half the tour. **ROUND 3 (the owner ran round 2's build `34704379023`):** *"You add the skip option and it's not a trough guide mean it got cut / I want a full process 1st to last without skip anything in this / At the end option to close and view again."* **Four causes, all in round 2's own code** (TROUBLESHOOTING §37): SKIP TOUR was the most visible thing on every card and one tap wrote all ten beats seen; the **pass-over law** skipped any beat whose control was not laid out at that instant, so the Packages box could arrive before the Flask preview did; two beats had no target when needed (the project-name box only where a switch would teach something, the tabs box only while the keyboard hid the bar); and the project picker closes the drawer the next beat lives in. **Now:** every beat waits, in order, and a later beat never jumps the queue; a tour card has **no button at all** and `markAllSeen` is **deleted**, so nothing can spend a beat the user never saw (one writer of the seen set, pinned by count); Back navigates instead of ending anything, and the same beat is there on return; while a beat waits **nothing is drawn**, which is the safety argument that replaces the skip; and the **finish card** is the only card with buttons — **VIEW AGAIN** (an empty seen set + navigate to the editor, where beat 1 lives) and **CLOSE** (in-memory, so a beat the guard passed is still owed). Beat 2 is now anchored on the drawer header in EVERY project state, beat 6 on the **tab bar as well as the reveal handle** (one id, two publishers), and the editor **reopens the drawer after a project pick** when the pure plan says the beat it waits for is behind it (`nextBeatIsInDrawer` → `tourWaitsInDrawer`). The one thing that can still pass a beat is the **stall guard**: 20 s, and only for a control that COULD be on this route and is missing (`waitingOn` + `anchorsPossibleOn(route)`, with `ChromeState.route` added for it) — never for a beat the user must travel to or wait an install out for, because timing those out cascades; a passed beat is never marked seen. **The no-nag law's "skippable with a single tap" is reversed for the tour by the owner's own instruction** (the slides keep SKIP) — deviation 13. **180 host cases green locally, and CI ✅ GREEN on them: `34707337429` on tip `0fcb3b6`, `conclusion: success`, job `build` 10m51s, assemble + `testDebugUnitTest` + `lintDebug`, zero error annotations, release APK 6,669,858 B (+5,288 B / +0.08% over round 2 — this round ADDED a finish card, a route-aware stall guard, a second publisher for beat 6 and a drawer reopen, so five kilobytes is the measured price of a guide that cannot be skipped), debug 25,676,356 B** (`CoachMarkPlanTest` 21, `GuideWiringTest` 18, `GuidePlanTest` 15, `TooltipPlacementTest` 10, `DemoProjectSeedTest` 7). **The device round must start from Settings → About → Reset tips**, and its rows are now G1-G28 (G28 is the optional stall-guard row). Sixteen deviations recorded in the part docs, not hidden. Records: [`docs/chat-phase45/`](docs/chat-phase45/README.md) (implementation record + both parts' `## Implementation`), JOURNEY §61, root README "The guide (Phase 45)". **ROUND 4 (the owner ran round 3's build `34707337429`):** *"It's good but i have 2 request- / 1. When the userland is installing and unpacking the user can not access any other other option and it will show a sweet massage of why can't access any other option / 2. Now the steps feel like an overlay on the botton so 1st click disappear the massage and i have to click 2nd time to really work but if someone don't click 2nd time it just cut off the flow of tutorial / So do something".* Request 2 is a bug in round 3's own mechanism (TROUBLESHOOTING §38): the overlay advanced on the **press** and left the tap **unconsumed**, trusting Compose to deliver the rest of the gesture to the control underneath — but the advance's own recomposition (`coachSeen` → `tourWaitsInDrawer` → the next box) rebuilds that control before the **lift**, and a `clickable` whose node was rebuilt is cancelled. So the box went away, the beat was spent, nothing opened, and the tour then waited in silence (deviation 14). **The mechanism changed, not the timing:** an anchor now publishes its control's **own click** beside its rect (`GuideAnchor.modifier(id, onClick)`; registry `rects` + `actions` + **`owners`**, where `withdraw`/`withdrawAction` are honoured only by the site that published — one id with two publishers that swap in the same recomposition, the bar and the handle, would otherwise let the leaving site wipe the arriving one), the PURE `GuideTapPolicy.targetFor` picks the most specific anchored click under the press (smallest box, ties on tour order — beat 6's bar-wide hole resolves to the tab under the finger; a tap on an unanchored spot inside the hole is left to the app), `isTap` refuses a drag that travels and lifts outside the hole (Chebyshev ≤ slop OR lift inside the box; a scroll spends no beat and performs nothing), and the overlay **performs the click, THEN advances**, swallowing the gesture so nothing can fire twice. Two anchors deliberately publish no click: the terminal's status chip (a label) and a Packages card whose language is already installed (`cardClick` = null when `isInstalled`, `onViewSetup` when a refusal is showing, else `onInstall`) — publishing a stale `onInstall` there would turn a tap into a reinstall nobody asked for. Beat 6's copy now says *"tap this handle to bring them back"* instead of *"swipe up"*: while a box is up the tour accepts a tap (the swipe still works with the tour over; `NavBarPolicy.revealOnSwipe` untouched). **Request 1 is a Phase 44 surface built beside the gate** — pure `SetupLockPolicy` in `SetupState.kt` (`ChromeOption`, `ChromeLockReason`, `ChromeLock`, `lock/reasonFor/sweetMessage/watchOption/option/editorChromeLocked/optionForRoute`): the userland's own install (DOWNLOADING/VERIFYING/EXTRACTING **and** `!facts.usable`) pauses all tabs but **Terminal**, and a package install the user asked for (`OutputRunState.installing && busy`, new field with ONE writer and two clears, reported through the new `EditorChromeState.installRunning`, cleared on dispose) pauses all tabs but the **Editor** plus ☰ / RUN ▶ / `ModalNavigationDrawer.gesturesEnabled` inside it. The law: **the surface that shows the install is never paused** (`watchOption`); `CHECKING`, settled stages (READY/FAILED/UNSUPPORTED) and an in-flight **upgrade** of a working prefix pause nothing; and the lock never weakens the gate beside it — `SetupLockPolicy` answers CHROME while `SetupGatePolicy.can` still answers CAPABILITY (`RUN_C`/`EDIT_FILE` allowed in every stage, so typing and `cc` never wait for a download). One sentence per reason (the download's carries the real percentage, never invented) shown once when the pause begins (`LaunchedEffect(chromeLock.locked)`, keyed on `locked` so three stages of one install do not stack three snackbars) and again on every refused tap; paused tabs are dimmed to 0.45 with a small `Icons.Default.Lock` **before** they are tapped, and the scaffold grew a `snackbarHost` (a line, not a wall, blocking nothing). The tour pauses with the chrome (`blockedByForeground … || chromeLock.locked`) because a box on a paused control could only be spent by a tap that merely shows the sentence. Deviations **17-20**; the lock is specified in [`docs/chat-phase44/PART_44_1_VISIBLE_SETUP.md`](docs/chat-phase44/PART_44_1_VISIBLE_SETUP.md) §"Phase 45 round 4 — the chrome lock", the tap fix in [`docs/chat-phase45/PART_45_2_COACH_MARKS.md`](docs/chat-phase45/PART_45_2_COACH_MARKS.md) §Round 4, owner-facing text in TROUBLESHOOTING §38. **193 host cases green locally** (`CoachMarkPlanTest` 25, `GuideWiringTest` 19, `SetupGatePolicyTest` 30, `SetupGateWiringTest` 23, `GuidePlanTest` 15, `TooltipPlacementTest` 10, `DemoProjectSeedTest` 7 = 76 guide/demo + 117 Phase 44) — and the harness note: round 3's pins on the tap handler all passed on code whose taps were dead on a phone, because they pinned the mechanism's *prose*; round 4 pins **which lambda each beat performs**, **which anchors must publish none**, and the **ORDER of two calls by index** (`perform` before `onAdvance`). **CI ✅ GREEN on the round-4 commit: `34711827176` on tip `e7759f1`, `conclusion: success`, job `build` 25 steps 11m29s, assemble + `testDebugUnitTest` + `lintDebug`, zero error annotations, release APK 6,675,258 B (+5,400 B / +0.08% over round 3 — an anchor-click registry with ownership, a pure tap policy and a chrome lock wired across five screens; whole phase +23,576 B / +0.35% over Phase 44 round 4), debug 25,692,976 B, mapping 56,258,512 B, v1.3.17.** **ROUND 5 (the owner ran round 4 and accepted it, with one correction):** *"The lock option is good but still it late user can switch before the start of userland download because is takes a little time to connect and user can switch task between them / Make it instantly after 1st open and others are ok"*. The gap was round 4's own exemption: `reasonFor` was **stage-keyed**, so the whole `CHECKING` window — the ledger read, the probe of `bin/pkg` and `bin/bash`, the reach for the network, the server's first answer — was open, and 44.1's launch divert only picks the *starting* tab, so one tap in that window landed the user on Packages reading *"not installed"* about tools already on their way. **The rule is now prefix-keyed:** `!facts.usable` and not given up on ⇒ paused, whatever the stage says. New `ChromeLockReason.USERLAND_STARTING` covers `CHECKING` and a `READY` the disk contradicts (transient — 44.1's C4 correction fails it) with the sentence *"Hang tight — CodeC is getting ready to set up its Linux tools. Other options are paused for a moment so this one-time setup finishes cleanly. The Terminal tab shows every step."* (no percentage: none exists yet, and 44.1's rule is never invent one). `FAILED`/`UNSUPPORTED` still answer NONE — a stopped setup has its own sentence, its own ⬇ retry, and **C still compiles offline** — and `facts.usable` still short-circuits BEFORE the stage table, which is what keeps an installed phone from flashing a pause at launch (`TerminalViewModel` builds the facts **synchronously from the disk** in its constructor: `MutableStateFlow(computeSetupFacts(setupTracker.state))` → `SetupGatePolicy.factsFor(prefixDir, phase, progress)`). Three consequences recorded as decisions, not accidents: `lock()`'s **defaults are now the paused case** (they *are* the first frame of a fresh install — a caller that has heard nothing yet must not default to open); a boot-time **repair** of an interrupted swap (`swapping` ⇒ `!usable`) is paused too, with the Terminal open where 44.2 logs the restore; and `lock`/`reasonFor` grew **`reducedStart`** (`MainActivity` passes `SafeMode.active`) — Phase 42.3's safe mode is exempt from the userland branch because a phone that crashed three times must still reach Settings (export all projects, report the crash), while an install the user ASKED for still outranks it. Nothing else moved: the watch surface is never paused, the editor keeps typing and `cc` while the *userland* is built, `SetupGatePolicy.can` is still the only answer about capability, and 45.2's 76 guide/demo cases are byte-identical (the tour simply pauses a few seconds earlier — deviation 20 doing its job). Specified in [`docs/chat-phase44/PART_44_1_VISIBLE_SETUP.md`](docs/chat-phase44/PART_44_1_VISIBLE_SETUP.md) §"Round 5 — *Make it instantly after 1st open*", owner-facing in TROUBLESHOOTING §39. **197 host cases green locally** (`SetupGatePolicyTest` 33: the first frame is paused incl. the probe window, a repair and a disk-denied READY; a usable prefix pauses nothing at ANY stage incl. an upgrade moving through all three; a reduced start pauses nothing while a package install still outranks it. `SetupGateWiringTest` 24: `FAILED, UNSUPPORTED` are the ONLY stages answering NONE, `facts.usable` short-circuits before the stage table, `MainActivity` passes `reducedStart = SafeMode.active`, the facts are built in the constructor. = 76 guide/demo + 121 Phase 44). The harness itself had to be **rebuilt from scratch** this round (the sandbox lost `/tmp`), which produced three reusable lessons: a harness runner must build a **fresh test-class instance per test method** (reusing one made five `DemoProjectSeedTest` cases fail on state leaked through a shared `TemporaryFolder` root — failures that looked exactly like production bugs), the shim `Assert` needs JUnit's **`assertEquals(Double, Double, delta)`** overload, and `@get:Rule` needs `AnnotationTarget.PROPERTY_GETTER` on the shim annotation. **CI on the round-5 commit is ✅ GREEN: `34714305062` on tip `6c3cfea`, `conclusion: success`, job `build` 25 steps 10m47s, assemble + `testDebugUnitTest` + `lintDebug`, zero error annotations, release APK 6,675,254 B (−4 B against round 4 — the whole change is one enum constant, a reordered `when`, one parameter and a sentence; whole phase +23,572 B / +0.35% over Phase 44 round 4), debug 25,693,272 B, v1.3.17.** **CI round 1 ✅ GREEN: `Build APK` `34698914219` on tip `3c597b2` — `conclusion: success`, job `build` 9m55s, zero error annotations; per `rule.md` §5 that is `:app:assembleDebug :app:testDebugUnitTest :app:lintDebug`, so the 50 new cases ran on real Gradle/JUnit/Robolectric and lint is clean. Artifacts `CodeC-IDE-release` 6,664,474 B (**+12,792 B / +0.19%** over Phase 44 round 4's 6,651,682 B — the whole guide's cost) / `CodeC-IDE-debug` 25,661,856 B (v1.3.17); the only annotations are the repo-wide Node 20 → 24 runner deprecations. Next: the owner's Phase 45 round (**G1-G40** — G29-G33 the one-tap rule, G34-G38 the chrome lock, G39-G40 its first-frame timing; G39 wants a fresh install with a slow or absent network, G40 an installed phone — on `Build APK` run <https://github.com/pabi277/CodeC/actions/runs/34714305062>, artifact `CodeC-IDE-debug`, after Settings → About → **Reset tips**) AND Phase 44's round 2 (R1-R8, then D1-D12, whose lock rows are G34-G40) on the same phone, then "Start Phase 46" (Projects, not folders).**
...
- **Phase 45** (the guide: five slides on first run + a ten-beat tour that cannot be skipped, where one tap does both halves and an install pauses the other options **from the first frame**) — 🚧 IMPLEMENTED through **round 5** on `arena/01a0955a-codec` (both parts; CI ✅ GREEN all five rounds incl. round 5's `34714305062`; round 2 made 45.2 one ordered tour, round 3 removed its skip, round 4 made each highlighted control **perform its own published click** and advance on the lift — a drag is not a tap, a bar-wide hole resolves to the tab under the finger, an installed card publishes nothing — and added the **chrome lock**: pure `SetupLockPolicy`, the watch surface never paused, a sweet sentence on every refused tap, and a package install pausing everything but the Editor; round 5 made the lock **prefix-keyed instead of stage-keyed** — paused from the first frame, with `USERLAND_STARTING` covering the probe window and a boot repair, `FAILED`/`UNSUPPORTED` and safe mode exempt, and a usable prefix short-circuiting before the stage table so an installed phone never flashes a pause; **197 host cases green locally**); **device round NOT run** — `docs/chat-phase45/DEVICE_ROUND.md` (G1-G40) — `docs/chat-phase45/`. The copy is vocabulary-pinned to the real tree (`GuideVocabulary` + `GuideTermProof`: a word that is not a real button label, file name or language fails the build), the anchors withdraw on dispose so a mark can never point at a control that is not on screen, and the two new Settings rows are audit rows **53-54**.
