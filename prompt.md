...
- **Phases 15–17 (Spck clone) are all IMPLEMENTED, device-rounded and MERGED
  ...
  - **Phase 18 — CodeCApi Device Capabilities ✅ COMPLETE & DEVICE-ACCEPTED
    ...
    `33468442063`; one red round fixed a lint ERROR; owner's §4 recipe PASSED
    on device — incl. codec-camera's dialog → CAPTURING → photo → OK path).**
    Five CLI scripts + wire ops on the existing OSC 1337 CodeCApi bridge:
    `codec-battery` (sticky `ACTION_BATTERY_CHANGED` → JSON), `codec-sensor`
    (accelerometer/gyroscope/light), `codec-tts` (app-lifetime
    TextToSpeech), `codec-camera` (runtime CAMERA park/resume +
    `TakePicture` via FileProvider → photo in
    ...
    notes) and §5.6 (device transcript). **Phase 18 CLOSED; do not revisit
    unless an identical symptom reappears with regression evidence.**
    Clean-room, client-only, no `[repo-build]`.
...
1. **Phase 18 is CLOSED (device-accepted 2026-09-01).** All spec'd work is
   done; no further code unless the owner observes a regression.
...
2. Phases 3–14 + 19 are closed (PRs #15/#23/#25/#26/#27/#28/#29/#30/#31/#32/#34
   ...
   **Phase 18 is complete & device-accepted on `arena/01a05b12-codec`
   (`4460306`, run `33468442063`) — CLOSED; there is no remaining spec'd
   implementation.** If the owner commands new work, re-verify its plan
   against current code, then implement it host-testably, commit + push the
   session branch, and let CI run. Never open/merge a PR without the owner's
   explicit word.
