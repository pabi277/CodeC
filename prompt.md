...
- **Phase 45** (the guide: five slides on first run + a ten-beat tour that cannot be skipped, where one tap does both halves and an install pauses the other options) — 🚧 IMPLEMENTED through **round 4** on `arena/01a0955a-codec` (both parts; CI ✅ GREEN all four rounds incl. round 4's `34711827176`; round 2 made 45.2 one ordered tour, round 3 removed its skip, round 4 made each highlighted control **perform its own published click** and advance on the lift — a drag is not a tap, a bar-wide hole resolves to the tab under the finger, an installed card publishes nothing — and added the **chrome lock**: pure `SetupLockPolicy`, the watch surface never paused, a sweet sentence on every refused tap, and a package install pausing everything but the Editor; **193 host cases green locally**); **device round NOT run** — `docs/chat-phase45/DEVICE_ROUND.md` (G1-G38) — `docs/chat-phase45/`. The copy is vocabulary-pinned to the real tree (`GuideVocabulary` + `GuideTermProof`: a word that is not a real button label, file name or language fails the build), the anchors withdraw on dispose so a mark can never point at a control that is not on screen, and the two new Settings rows are audit rows **53-54**.
