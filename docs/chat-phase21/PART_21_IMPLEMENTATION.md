...
## 9. D.3 PASSED + owner redirect: D.4 CANCELLED, TCC is the default (2026-09-03)

**Owner: "Pass"** — D.3 device acceptance is **CONFIRMED**. Then, when D.4's
blocker was raised (see below), the owner redirected:

> *"I want you to remove the option of compiler Setting and make the tcc
> default but if need user can install gcc"*

### Why D.4 stopped before deleting anything

Starting the D.4 checklist surfaced a dependency the spec never modelled:
**`cc` is not clang — it is CodeC's own TCC frontend**, generated by
`ShellBootstrap` and driven by `$TCC_BIN`/`$TCC_BUNDLE`. Phase 20.1 works hard
to keep it that way: `apply-recipe-overrides.sh` *strips `bin/cc` from the
clang deb* so `pkg install clang` can never clobber it (the **cc invariant**,
D5/D15), and the owner's own round-5 device run verified `command -v cc` →
tcc 0.9.27.

Deleting the TCC assets would therefore have: broken `cc` in the terminal
(exit 127), broken every existing `project.json` whose build line says
`cc main.c -o bin/app`, removed the first link of `CompilerService`'s
auto-chain, and orphaned the Packages screen's built-in TCC entry. Four
behaviour decisions, not mechanical deletions. Raised instead of executed.

**Outcome: D.4 (Remove TCC Entirely) is CANCELLED by owner direction.** TCC is
not legacy — it is the default C compiler. `PART_21_4_REMOVE_TCC.md` is marked
cancelled; the TCC link-order invariant (`-o` last) **stays in force** in
`rule.md` §6 and `prompt.md`, and this record's §3.5/§8 notes about "retiring
TCC" are superseded by this section.

### D22 — Settings → Compiler Engine picker removed

The four-way dropdown (Auto / Built-in TCC / Bundled Clang / Termux) and its
"Engine notes" blurb are gone, replaced by one honest read-only line:

> **Compiler** — C compiles with the built-in TCC by default — offline,
> instant, no download. C++ and advanced C11/C17 code need the full LLVM
> toolchain: install it from Packages (or run `pkg install clang` in the
> terminal) and RUN ▶ offers it automatically when a file needs it.

- `SettingsManager`: `COMPILER_BACKEND` key, `compilerBackendFlow` and
  `setCompilerBackend` deleted — the preference is no longer persisted.
- `CompilerService`: the `BACKEND_*` constants stay as the **internal**
  fallback chain (embedded → bundled → Termux) but `BACKEND_AUTO` is now the
  only value any caller passes. Every user-facing error string that said
  *"switch Settings → Compiler Engine → …"* now points at Packages /
  `pkg install clang` instead — those instructions referenced a control that
  no longer exists.
- The **Built-in Compiler** and **Termux Engine** status cards stay: they are
  diagnostics, not a selector.

### D23 — the registry follows the same rule: TCC first, clang on demand

The C profile is reverted from the gcc/clang line introduced in §3 to the
built-in frontend, and — importantly — **its install gate is removed**:

| | before (§3/§7) | now |
|---|---|---|
| `.c` | `requiredPackage = "clang"`, `gcc $SRC -o $OUT -lm` | `requiredPackage = null`, **`cc $SRC -o $OUT`** |
| `.cpp` | `requiredPackage = "clang"`, `g++ …` | unchanged — TCC cannot build C++ |

So a `.c` file now runs **offline, out of the box, with no prompt and no
90 MB download** — which is the whole point of shipping TCC. C++ keeps its
gate because TCC genuinely cannot compile it.

`-o` is once again the **last** argument on every C line, restoring the TCC
link-order invariant. `TerminalHandoff.compileParts`/`projectFileParts`, the
`c-microservice` preset and the C scaffold comment all follow suit; C++ still
uses `g++ … -lm` from the installed toolchain.

The D20 command gate is unaffected and now *helps*: `cc` is not any profile's
`probeBinary`, so a `cc`-based build line never triggers an install prompt,
while `g++ server.cpp` or `python3 app.py` still do.

### Tests

`LanguageRegistryTest` / `LanguageRunPlannerTest` / `TerminalHandoffTest`
re-pinned to the TCC-default behaviour, plus new guards:

- `c_uses_the_builtin_tcc_and_is_never_gated_behind_an_install`
- `a_c_file_runs_even_when_nothing_is_installed`
- `the_c_build_line_keeps_the_tcc_link_order_with_o_last`
- `only_the_c_profile_compiles_through_the_cc_frontend` (replaces the old
  "no profile may use cc" guard, which this redirect inverts)
- `the_registry_never_gates_a_language_the_app_ships_itself` (C, shell, HTML)
- `the_cc_frontend_never_triggers_an_install_prompt`
- `cpp_without_a_toolchain_installs_clang_not_gcc`

### Phase 21 final shape

D.1 registry ✅ · D.2 install gate ✅ · D.3 device PASSED ✅ · **D.4 cancelled**
· D22/D23 (this section) ✅. TCC is the default C compiler; every other
language installs on demand from the Phase 20.1 repository.
