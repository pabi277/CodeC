# CodeC Phase 53.2 — Jank budget and measured first paint
...
the icon to a usable screen. Today there is no splash (52.1's finding) and no
...
device round (Phase 36's three follow-up regressions, Phase 48's blink). 53.2
...
Each is recorded **before** (on the merged phase-52 build) and **after**, in a
...
own evidence — not a silent tweak inside 53.2.
...
1. **Skeletons instead of blanks** in the three remaining loading surfaces
   (file tree, git screens, package list) — 52.3 already did the hub; a blank
...
- `SkeletonCoverageTest` (~7, source scan): the file tree, the git screens and
  ...
  item count as its loaded list (scroll-position safety — 52.3's rule).
...
- `PHASE51_53_UX_RESEARCH.md` §2.6 (the instruments exist), §3.6.
