# CodeC Phase 39 — File system strength (open any folder, never crash)
...
```text
  39.1  "Open folder" that cannot crash (safe SAF walk)
  39.2  Open a folder as a project in place (ProjectRef + link/sync)
```

| Part | Title | Effort | Status |
|---|---|---|---|
| [39.1](PART_39_1_SAFE_FOLDER_WALK.md) | Crash-proof folder import + cancel/progress | M | 📋 PLANNED |
| [39.2](PART_39_2_OPEN_FOLDER_AS_PROJECT.md) | Open a folder as a project, in place | L | 📋 PLANNED |
...
- **The one way in is a copy, and it is fragile.** Hub → *Open folder*
  ...
  the owner's second sentence, and it is a plausible, code-visible cause; 39.1
...
- **The picker is half-right: the contract is correct, the persistence is
  ...
  import-by-copy is the only story. 39.2 adds the call; 39.1 does not (one-shot
...
- **Storage is already two-track, and 39 must not pretend otherwise.**
  ...
  **already declares and already offers** `MANAGE_EXTERNAL_STORAGE`
  ...
  `File` access to `/sdcard` is a supported path here, not a fallback: 39's
  walker therefore takes a `File` **or** a `Uri` and never proposes changing that
  permission story (43.3 audits the list instead, honestly).
...
1. **`noexec` is physics.** A project's *sources* may live outside
   ...
   `python3` child run against it) runs from app-private storage. 39.2's
...
5. (39.2) Open a folder that already has code from `Download/` (or an SD card)
...
6. (39.2) Kill the app and reopen: the linked folder still works (persisted
...
PASS = 1-4 for 39.1; 5-6 additionally for 39.2.
```
...
- **Editing an SD-card project fully in place** (no mirror) — needs every
  ...
  `exec`/`cc`, and makes `git status` on a slow provider unusable. 39.2's
...
- **Watching the linked folder for external changes** (`FileObserver` /
  `ContentObserver` on the tree) — deferred; 39.2 ships an explicit
