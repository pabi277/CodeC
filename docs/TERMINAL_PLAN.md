...
| **7** | Multi-terminal sessions: `TerminalSessionManager` (N sessions, each PTY + emulator); session drawer / switcher / "+" button; route `CodeCApi` per session; persist session list across screen changes (not across app restart — matches Termux) | [client-only] | 6 | 🔶 **IMPLEMENTED (2026-08-28, `arena/01a048df-codec`)** — CI compile-green run `33185424586`; dropdown switcher (D9), per-session CodeCApi collectors, protocol unchanged (D4 evidence); unit tests written but not yet CI-executed; **device recipe still pending** (see `docs/chat-phase7/`). |
