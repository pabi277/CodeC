...
42. **Phase 31 — IntelliSense as Packages, 31.1 STARTED (2026-09-07, `arena/01a07baa-codec`, owner: "Start phase 31" — see also the "two phases at once, different chats" note in §9 below).** The README scope is Acode-style: `Packages → pkg install clangd`, the editor attaches **one** LSP per active file, no Ace/JS plugin runtime. The phase was already 📋 PLANNED from §39 above. This commit ships **31.1's engine + host tests only** — the production sora-LSP wire is its own follow-up (see §3.1 of the part doc), the same partial-pass pattern as 29.1/29.2/30.0/30.1.
    - **Gradle:** `io.github.rosemoe:editor-lsp:0.24.6` (same BOM/version as sora-editor + language-textmate; T1 = "never pin a second sora version"); binary dep only, no source vendored; LGPL-2.1 (sibling of sora-editor the owner accepted in 25.2; new `assets/licenses/SORA_EDITOR_LSP_LGPL.txt` + Settings → About updated).
    ...
    - **APK delta (estimate, matches the sora editor-lsp aar size):** +528 157 B raw AAR + ~3 000 B Kotlin DEX (4 small files) + 4 B asset notice. Phase 29.1's owner-accepted deviation lesson: state the delta BEFORE the device round so the owner sees the same number on the artifact.
    - **Records:** `docs/chat-phase31/` (README updated to 🚧, `PART_31_1_EDITOR_LSP.md` rewritten with §1.1–§1.2 / §2.1 device recipe / §3 follow-ups; 31.2 and 31.3 stay 📋 PLANNED). Status of CI: **not yet run** — this commit is on `arena/01a07baa-codec` (the session's branch), the `Build APK` workflow is the executor of record (Gradle 9.3.1 + AGP 9.1.1 + sora 0.24.6); pre-validation was a local JVM smoke only. **No PR/merge without the owner's explicit command** (standing rule).
