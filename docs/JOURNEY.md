...
43. **Phase 31.2 + 31.3 — IntelliSense install cards SHIPPED (UX SHELL, 2026-09-07, same day, `arena/01a07baa-codec`)** — owner chose scope "just 31" and dep resolution "keep the gate, land 31.2/31.3 as the UX shell with the no-op provider" (the production sora `LspEditor` wire stays the §3.1 follow-up).
    ...
    - **CI:** **GREEN** on tip `d107470` (`Build APK` run `34123490652`, 8 m 15 s; assemble + `:app:testDebugUnitTest` + `:app:lintDebug` + `:bench:assembleRelease :bench:testDebugUnitTest`). First push (`29c7d77`) failed with `Unresolved reference 'SystemBinaryProbe'` at `MainActivity.kt:277` because the import was missing — a one-line `import com.codeci.ide.ui.editor.lsp.SystemBinaryProbe` fix in `d107470` made it green. Records: `docs/chat-phase31/PART_31_2_CLANGD.md` + `PART_31_3_PY_JS.md` flipped to ✅ CARD SHIPPED with the §3.1 follow-up card mirrored from PART_31_1; README flipped to 🚧 **31.1 + 31.2 + 31.3 IMPLEMENTED**; rule.md Phase 31 partial-state bullet updated. **No PR/merge without the owner's explicit command** (standing rule).
