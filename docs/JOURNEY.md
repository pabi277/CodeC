...
`33730937920` green). Phase 23 is IMPLEMENTED & CI-GREEN on
`arena/01a06662-codec` (`Build APK` `33735687876`)
(owner: "Start Phase 23"): B.1 inline PTY input (remove `OutputInputRow`) +
B.2 context-aware run keys (`↵ Enter`/`Ctrl+C`/`Tab`/history), device recipes
pending.**
...
32. **Phase 23 — Interactive Run UX (2026-09-03, `arena/01a06662-codec`, owner: "Start Phase 23").** Two client-only parts, both implemented in one pass; **CI green `33735687876`** (one for-cause red round `33735482625` first — a missing `kotlinx.coroutines.flow.update` import in `EditorViewModel` — fixed in the same commit set).
