...
`33730937920` green). Phase 23 is IMPLEMENTED on `arena/01a06662-codec`
(owner: "Start Phase 23"): B.1 inline PTY input (remove `OutputInputRow`) +
B.2 context-aware run keys (`↵ Enter`/`Ctrl+C`/`Tab`/history), CI pending.**
...
32. **Phase 23 — Interactive Run UX (2026-09-03, `arena/01a06662-codec`, owner: "Start Phase 23").** Two client-only parts, both implemented in one pass; **CI pending at commit time.**
