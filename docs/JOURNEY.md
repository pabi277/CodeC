...
45. **Phase 32 — Phone canvas (2026-09-09, `arena/01a083fc-codec`, owner: "Start phase 32").** The three client-only parts that end the "postage stamp of code" (`PHONE_UX_ANALYSIS.md` changes 7–8, 11) — typing + Keys + chips + a permanent 5-tab bar left almost no editor. **32.1** auto-hides the bottom 5-tab bar while the EDITOR is the destination AND a keyboard occupies the bottom (the system IME, or CodeC Keys); a thin handle (pill + "Show tabs") — tap OR swipe-up ≥32 dp — reveals the bar, sticky until the user leaves the editor; other tabs keep the pre-32 rule (bar hidden only while the soft IME is up). The decision is pure (`ui/editor/NavBarPolicy.kt`); the editor reports its keyboard through a tiny StateFlow bridge (`ui/editor/EditorChromeState.kt` — `EditorScreen` sets it from the existing `codecKeysUp` gate and clears it on dispose). **32.2** needed NO code — it was already satisfied by 28.2 (`suppressKeysVariant` hides the utility-keys strip when Keys is up), 28.3 (chips ride as row 0 above Keys via `StripContext.Suggestions`), and the new 32.1 nav-hide; the one-meaning-row is code → chips → Keys with no bar. **32.3** hardened the Output Panel tap-to-line: a diagnostic naming a compiler temp (`source_<stamp>.c`) or a file that is not a real file under the run's folder used to be IGNORED; `EditorViewModel.openOutputDiagnosticFile` now resolves through the pure `ui/editor/OutputDiagnosticTarget` and FALLS BACK to the ACTIVE file (the run was launched from it), so the caret lands in the user's file, never on a nonexistent temp copy. The first-RUN output expansion was already Phase 11 behaviour (unchanged).
    ...
    - **Gate:** `Build APK` CI green + owner device round (`TROUBLESHOOTING.md` §16): bar gone with Keys/IME open, back on navigate-away, handle restores without losing the buffer; one row of chips above Keys; RUN shows output and a compile-error tap lands on the right line in the OPEN tab.
