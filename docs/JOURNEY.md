...
40. **Phase 29 (VS Code colour via TextMate) 🚧 IMPLEMENTED (2026-09-05, owner: "Start phase 29" — all three parts in one build; CI + device round pending at write time).** The same move as 25.2, one layer up: **depend on sora's `language-textmate`, retire our tokenizer from the hot path.** The widget was already sora; now the ANALYZER is too — VS Code's own grammar format (tm4e engine, MIT grammar assets), replacing the ~690-LOC `MultiLanguageSyntaxHighlighter` regex that Go/Rust/PHP/Ruby/Lua files never even had (they coloured as TEXT).
    ...
    - **CI:** run 1 failed one theme test (Robolectric's per-method Application recreation staled sora's AssetsFileResolver → applyTheme degraded to the previous theme); fixed with a refreshable resolver + by-reference theme switching (`a19b033`), a dropped-field compile slip fixed (`0e64b87`), **run 3 (33983164706) GREEN**. APK delta vs `main` NOT yet measured — the sandbox GitHub token expired right after the green run (see rule.md §9); measure the artifact before the device round.
