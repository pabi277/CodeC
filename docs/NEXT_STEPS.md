...
**Last updated:** 2026-09-06 · **Head: Phase 29 (VS Code colour / TextMate) 🚧 IMPLEMENTED, CI GREEN — device round 1 BLOCKED on crash 2** (owner: "Start phase 29") — all three parts (29.1 `language-textmate` core + themes, 29.2 language parity via the `LanguageType` split, 29.3 regex retirement) built, CI green (`61c39cc`); crash 1 (CME on file open, registry-monitor + locked-createLanguage + main-thread switch) FIXED and regression-tested; **crash 2 = NEW main-thread exception during MEASURE into the editor — exception type still unknown.** Root cause of the undiagnosable tail-only reports: the crash overlay showed only the last 6000 bytes of the accumulating crash log; FIXED in `591be79` (header-first frame-capped records + bounded file; overlay shows the newest record FROM ITS HEADER — COPY ALL is now complete). Robolectric editor-screen repro GREEN (pure-JVM measure clean → device-only factor; kept as integration smoke). **NEXT: owner installs the newest green APK, reproduces crash 2, relaunches, COPY ALL → paste → root-cause + fix.** **APK delta MEASURED +2.10 MiB (22.0 → 24.2 MB) — over the +1.5 MiB budget by ~0.6 MiB (engine chain joni+jcodings+gson+tm4e ≈ 1.95 MB; assets ≈ 250 KB) — owner verdict still pending at the device round / merge gate.** Gate: owner device round (`TROUBLESHOOTING.md` §12; keystroke p95 ≤ 16.7 ms) — records in [`chat-phase29/`](chat-phase29/README.md). **No PR/merge until the owner's command.** Remaining queue: 30 snippets/Emmet, 31 LSP Packages, 32 phone canvas, 33 first-hour (all 📋 PLANNED, docs only). 28.3/28.4 remain the Keys remainder.
