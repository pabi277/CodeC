...
**Last updated:** 2026-09-09 · **Head: Phase 32 (Phone canvas) ✅ DEVICE-PASSED + Phase 33 RUN ▶ chooser implemented on `arena/01a083fc-codec`** — **32.1** auto-hides the 5-tab bar in the editor while Keys/IME is up, with a thin tap/swipe handle to reveal it (pure `NavBarPolicy` + `EditorChromeState` bridge); **32.2** needed no code (28.2/28.3 already give chips-as-row-0; 32.1 removes the nav); **32.3** makes a diagnostic tap land in the user's file (pure `OutputDiagnosticTarget`). **CI `34305070875` ✅ GREEN first try (tip `b5feb55`); device round `TROUBLESHOOTING.md` §16 ✅ PASSED** (owner: "All test passed on device"). **Phase 33 first item done (owner: "when user hit run it asks index/main file or the file currently opened"):** RUN ▶ now asks — a two-button chooser (pure `ProjectRunTarget`) — when a project has a main/index file (`ProjectConfig.entry`) that differs from the open file; `runMainFile`/`runCurrentFile` carry out the choice, and the ViewModel's new `runFile(context, target)` runs the chosen file without switching tabs (target-aware registry decision, dirty-tab flush, diagnostic basename, install-resume). Tests: `ProjectRunTargetTest`. **Owner bug fixed (2026-09-09):** a C/Python file inside an HTML project now RUNs in the panel (was: always previewed `index.html`) — pure `ProjectRunTarget.isRunnableSource` gates both the editor dispatch and the ViewModel's web-project early-return (`TROUBLESHOOTING.md` §17). **CI `34307172630` ✅ GREEN first try (tip `788ba46`).** **Not merged — the owner merges (§3).** Records: [`chat-phase32/`](chat-phase32/README.md), [`chat-phase33/`](chat-phase33/README.md), JOURNEY §45/§46. Remaining: 33.1–33.3 first-hour tiles/hub/copy; 28.3/28.4 remain the Keys remainder.
