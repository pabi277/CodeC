...
**Last updated:** 2026-09-05 · **Head: Phase 29 (VS Code colour / TextMate) 🚧 IMPLEMENTED** (owner: "Start phase 29") — all three parts (29.1 `language-textmate` core + themes, 29.2 language parity via the `LanguageType` split, 29.3 regex retirement) built on the session branch: sora's TextMate engine (same 0.24.6 BOM) now powers the editor analyzer with 24 MIT grammar assets + Dark+/Monokai/Dracula/GitHub-Dark themes (default **VS Code Dark+**), lazy grammar loading + background warm-up, host tests incl. the analyzer-swap law (Robolectric against the real assets). **Gate: CI green + owner device round** (`TROUBLESHOOTING.md` §12; budgets keystroke p95 ≤ 16.7 ms, APK delta ≤ +1.5 MiB) — records in [`chat-phase29/`](chat-phase29/README.md). **No PR/merge until the owner's command.** Remaining queue: 30 snippets/Emmet, 31 LSP Packages, 32 phone canvas, 33 first-hour (all 📋 PLANNED, docs only — [`chat-phase30/`](chat-phase30/README.md) … [`chat-phase33/`](chat-phase33/README.md); research: [`OSS_REPLACEMENT_RESEARCH.md`](OSS_REPLACEMENT_RESEARCH.md), [`PHONE_UX_ANALYSIS.md`](PHONE_UX_ANALYSIS.md)). 28.3/28.4 remain the Keys remainder.
