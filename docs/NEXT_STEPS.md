...
**Last updated:** 2026-09-09 · **Head: Phase 32 (Phone canvas) 🚧 IMPLEMENTED on `arena/01a083fc-codec` (owner: "Start phase 32")** — **32.1** auto-hides the 5-tab bar in the editor while Keys/IME is up, with a thin tap/swipe handle to reveal it (pure `NavBarPolicy` + `EditorChromeState` bridge); **32.2** needed no code (28.2/28.3 already give chips-as-row-0 with the utility strip suppressed; 32.1 removes the nav); **32.3** makes a diagnostic tap land in the user's file — `source_<stamp>.c` / non-resolvable names fall back to the active file (pure `OutputDiagnosticTarget`). Tests: `NavBarPolicyTest` + `OutputDiagnosticTargetTest`. Local pre-validation (Temurin 25 + kotlinc 2.4.10) passes and caught the `toRelativeString` direction bug pre-push; CI (`Build APK`) is the executor of record and the device round is `TROUBLESHOOTING.md` §16. **Not merged — CI green + owner device round are the gates (§3).** Records: [`chat-phase32/`](chat-phase32/README.md), JOURNEY §45. Remaining: 33 first-hour UX; 28.3/28.4 remain the Keys remainder.
