...
**Last updated:** 2026-09-10 · **Head: Phases 38-43 📋 PLANNED (docs-only, no code) — the owner's six "before I share the app" ideas became six researched phases, and Phase 37's merge stays HELD at the owner's command.** Rows → phases: **38** git (the clone error must show *inside* the dialog, `isAvailable()` must be called before a clone is offered, a push result card that separates "on the remote" from "committed here", and `POST /user/repos` for "publish this folder"); **39** file system (bounded `Throwable`-safe folder walk — the crash is a `StackOverflowError` escaping an unbounded `copyDocumentChildren` caught only as `Exception`; then open-folder-as-project via `ProjectLink` + a *persisted* SAF grant); **40** temp outputs + one ignore policy (`RunArtifacts`/`TempGc`, `RepoHygiene`'s ~60 patterns incl. `.codec/`, the user's `.gitignore` always winning, enforcement inside `stageAll`); **41** feedback (`FeedbackDraft` → `https://wa.me/<digits>?text=` with the number as a Settings value, three fallbacks, `GitRedactor` plus its own pattern table, no telemetry); **42** identity (original adaptive mark with a real `monochrome` layer + `ic_stat_codec` for the notifications, and the "Termux Engine" *card* deleted while `TermuxCompiler`'s fallback and `RUN_COMMAND` stay); **43** share-readiness (release-signed non-debuggable artifact + `app-v*` tags + `UpdatePolicy` reusing `UserlandInstaller`'s sha256/resume/stage discipline because `releases/latest` today resolves to `userland-v1`; measured R8 + `splits.abi` **plus** an `assets/tcc` exclude, since splits don't filter `assets/`; real backup rules — today the XMLs are AS samples, so `files/usr` and the `git_token` DataStore are backup-eligible; a crash-loop guard added to the existing `CrashReportOverlay`; `exportAllZip` over **both** project roots). **Implementation order 42 → 40 → 38 → 41 → 43 → 39, and 40.1 before 39.2** so a linked folder never receives CodeC's build outputs. Docs: [`PHASE38_43_ROADMAP.md`](PHASE38_43_ROADMAP.md), [`PHASE38_43_OSS_RESEARCH.md`](PHASE38_43_OSS_RESEARCH.md), [`chat-phase38/`](chat-phase38/README.md) … [`chat-phase43/`](chat-phase43/README.md), JOURNEY §52. **CI for the planning commit is ✅ GREEN — `Build APK` `34433912076` on tip `8d365c4` (6 m 18 s; `CodeC-IDE` 24 847 809 B, 97 B *under* the previous tip's build with no `app/` file changed — which is exactly why 43.2 sets a ~0.1 % noise floor before it claims any size win). Nothing here is implemented; each phase starts on the owner's "Start Phase N", and Phase 37's merge stays HELD at the owner's command.**
