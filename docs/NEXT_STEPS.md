...
**Last updated:** 2026-09-03 · **State (head):** **Phase 22 (editor smoothness + IME-anchored keys) 🟡 IMPLEMENTED & CI-GREEN on `arena/01a065a0-codec` (`f2cee13`, run `33717680783`) — DEVICE PASS REQUIRED, not merged.** A.1: debounced 80 ms off-thread syntax highlight (`HighlightedCode` + `EditorViewModel.highlighted`, with an inline fallback whenever the snapshot is stale so colors are never wrong), a decoration fast path, narrowed `tabViews`/`completionItems` recomposition keys and a cached gutter string. A.1's scroll-model rewrite is **deferred**: at Compose BOM 2024.09.00 the `scrollState` parameter exists only on the `TextFieldState` overload of `BasicTextField`, and migrating off `TextFieldValue` would rewrite the undo/auto-indent/tab/find pipeline. A.2: the keys row now renders as the last child of the `imePadding()`'d column while the soft keyboard is up, so it rides flush above the keyboard (docked Phase 16 position when closed); the key sets were already language-adaptive. A.3: `imePadding()` on `EditorScreen`'s root column only — `enableEdgeToEdge()` and `adjustResize` were already in place, and Terminal/Settings insets are untouched. New tests: `EditorHighlightCacheTest` ×8. **Owner action: run the three §4 device recipes in `docs/chat-phase22/`, then merge.** Next after that: Phases 23 → 24.
