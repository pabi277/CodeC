**Last updated:** 2026-09-10 · **Head: Phase 41 (Feedback that reaches you, WhatsApp-first) — 🔧 IMPLEMENTED on `arena/01a08cc6-codec`, ✅ CI GREEN on the first push** (`Build APK` `34525080153` on tip `8fdbe6a`, 6 m 15 s, artifact `CodeC-IDE` 24 992 107 B = +0.24 % vs `main`) — **device round pending (the owner's gate).** What shipped, both parts in one build: **41.1** `FeedbackDraft` — the pure report builder (fixed skim layout, redaction-before-budget via its token-shape table + `GitRedactor`, `~proj/`/`~home/`/`~app/` path shortening, the 1 800-char WhatsApp budget that never cuts the user's text) + strict E.164 `normaliseNumber` (ITU country-code table as data) + `wa.me`/`mailto`/GitHub-issue URL builders (UTF-8 percent-encoding pinned with Devanagari/Bengali/emoji fixtures); **41.2** the Settings **Feedback & Support** section (12th, after About) — text field, two ephemeral checkboxes (log off by default, crash prefilled when present), CHAT ON WHATSAPP (a `com.whatsapp` presence probe means a WhatsApp-less device gets copy + the number, never a dead link), COPY/EMAIL/GITHUB fallbacks (repo verified public), the owner's reply-to store (`FeedbackStore`, 2 keys, empty = hidden; `DEFAULT_WHATSAPP_NUMBER` is Phase 42's one-line "ship the number?" decision point), and the honest three-line disclosure above the checkboxes. `CrashLog.newestRecord` is now the ONE read of `crash-log.txt` (extracted from `CrashReportOverlay`), and `OpenInBrowser.openOrCopy` is the shared open-or-copy policy (the Phase 37 share row uses it too). Tests: 50 new host cases + `SettingsAuditTest` (12 sections) + `SettingsKeysHaveReadersTest` (4 stores) — **61/61 pre-validated on the Phase 40.4 local harness** (5 test-side bugs caught pre-CI). Device runbook: `chat-phase41/DEVICE_TEST_PLAN.md` (8 checks). **Next action: CI green → the owner's device round (checks 1–8) → "Start Phase 42"** (share-readiness — where the owner decides whether `DEFAULT_WHATSAPP_NUMBER` ships filled). Queue: 42 → 43.
