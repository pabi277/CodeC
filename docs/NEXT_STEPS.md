...
**Last updated:** 2026-09-09 · **Head: Phase 33 remainder (first-hour UX) IMPLEMENTED on `arena/01a085e0-codec`** — **33.1** first-run tiles: a fresh install opens on a welcome with **three starter tiles only — C / Python / HTML** — driven by a DataStore `first_launch_complete` flag (default false); pure `ui/projects/WelcomeStarters.kt` maps each tile to the wizard's own `ProjectScaffold` type + entry file, `ensureProject` is an idempotent create-or-open, and `MainApp` renders `WelcomeScreen` instead of the whole Scaffold while the flag is false (no bottom bar — "three tiles only"), saving the launch state **before** flipping the flag so the shell that replaces the welcome opens the starter file and the next launch opens it too. RUN ▶ needs no new code (C → TCC no dialog; Python → the Phase 21 install gate only when `python` is missing; HTML → preview); Settings → About gained a "Show the welcome screen again" reset. **33.2** the Packages hub now opens on a **Languages & IntelliSense** section (python/nodejs/clang/TCC + the 8 `intellisense-*` cards) with **Unix tools** collapsed by default — pure `PackageSection` + `PackageCatalog.sectionOf(item)` in `ui/modules/ModuleCatalog.kt`, and `ModulesScreen` swaps the flat list + category chips for the two sections (search still flattens them; install/run wiring unchanged, so INSTALL python still streams `pkg install -y …` in the Terminal). **33.3** identity copy: README + Settings → About now say "**write and run C, Python, JavaScript, and HTML on your phone. C works offline with no setup.**", and the empty Projects state points at the three starters instead of a blank list. Tests: `WelcomeStartersTest` (5), `PackageSectionTest` (5). **CI `34346424311` ✅ GREEN first try (tip `5db9e33`, 5m23s).** Records: [`chat-phase33/`](chat-phase33/README.md), JOURNEY §47. **Gate: owner device round (`TROUBLESHOOTING.md` §24).** Remaining: 28.3/28.4 Keys remainder.
