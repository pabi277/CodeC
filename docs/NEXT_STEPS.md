...
**Last updated:** 2026-09-04 · **Current head: Phase 25.2 (sora-editor integration) IMPLEMENTED, awaiting CI + owner device round (on `arena/01a06b20-codec`)** — owner's "Start Phase 25" ran the bench on-device and exported the full sheet. C-sora passed EVERY budget on both corpora (keystroke p95 14.5–16.6 ms, fling ≤3.1 % jank/0 bad, drag p95 ≤17.9 ms, completion p95 ≤22.5 ms, cold open ≤56 ms); C-now missed every bench.c budget (~400 ms/keystroke, 100 % jank — the owner's complaint, measured); C-compose2 hit the predicted whole-window recomposition trap (~36 ms frames, 100 % jank). **25.2 (Sora integration) CHOSEN — starts on the owner's "Start Phase 25.2"; 25.3 ❌ CANCELLED.** Decision table `docs/EDITOR_MOBILE_RESEARCH.md` §3.1; raw numbers `docs/chat-phase25/PART_25_1_SPIKE_BENCH.md` §4.5; JOURNEY §34. **No PR/merge until the owner's explicit command.**

**State (head):** **Phase 25.2 (sora-editor 0.24.6 as the edit core) ✅ COMPLETE & DEVICE-ACCEPTED on `arena/01a06b20-codec` (tip `c6b3ed2`; nine CI rounds, all history in PART_25_2 §CI) — owner "All passed" (round 4, build `c54228d`) (tip `08fb542`, run `33860045301`: `:app` compiles against sora incl. release, 11 new host tests pass, CodeC-IDE artifact 20.84 MiB = +0.55 MiB vs 25.1 — four device rounds: 1 crash (bridge echo race, §4.1) → 2 crash (REAL root cause via in-app crash report: applyDefault/constructor NPE, §4.3) → 3 recipe PASS + drawer-gesture & sora-native-completion fixes (§4.4) → 4 "All passed" (§4.5). All merge gates SATISFIED 2026-09-04 — owner: LGPL-2.1 "Yes" + "Merge" → **PR #49** (squash-merged; includes the bench CI wrapper removal — Phase 25 CLOSED) — merged as `101a560`; `main` CI `33869286080` GREEN, bench artifact gone).** Owner: "Start Phase 25.2" (2026-09-04). Widget-only swap per `docs/chat-phase25/PART_25_2_SORA_PATH.md` §4 implementation record: `CodeEditor` in an `AndroidView` behind the new `SoraEditorHost` bridge (`ui/editor/sora/` — `CodeCScheme` theme map, `CodeCAnalyzer` full-retokenize v1 on `SimpleAnalyzeManager`, `CodeCLanguage` with no-op completions/pure indent+pairs), the VM stays THE source of truth (tabs, dirty, autosave, VM-canonical undo with sora's stack disabled, find via `EditorSearcher`, keys strip, completions, hardware shortcuts). Phase 22.1 Compose highlight pipeline deleted from screen+VM. LGPL-2.1: binary dependency only; About attribution + `assets/licenses/SORA_EDITOR_LGPL.txt` shipped; **owner's explicit LGPL acceptance + APK-delta re-measure still gate the merge**. Known regressions recorded: diagnostics tap-popup (surface died), completion popup bottom-anchored. Host tests: `CodeCThemeMapTest` ×5, `CodeCLanguageLogicTest` ×6. No PR/merge without the owner's command.
