...
**State (head):** **Phase 25.2 (sora-editor 0.24.6 as the edit core) IMPLEMENTED & CI-GREEN on `arena/01a06b20-codec` (tip `08fb542`, run `33860045301`: `:app` compiles against sora incl. release, 11 new host tests pass, CodeC-IDE artifact 20.84 MiB = +0.55 MiB vs 25.1 — device round 1 found a crash on editor tap, fixed per PART_25_2 §4.1 (selection-echo guard + crash log); awaiting device round 2).** Owner: "Start Phase 25.2" (2026-09-04). Widget-only swap per `docs/chat-phase25/PART_25_2_SORA_PATH.md` §4 implementation record: `CodeEditor` in an `AndroidView` behind the new `SoraEditorHost` bridge (`ui/editor/sora/` — `CodeCScheme` theme map, `CodeCAnalyzer` full-retokenize v1 on `SimpleAnalyzeManager`, `CodeCLanguage` with no-op completions/pure indent+pairs), the VM stays THE source of truth (tabs, dirty, autosave, VM-canonical undo with sora's stack disabled, find via `EditorSearcher`, keys strip, completions, hardware shortcuts). Phase 22.1 Compose highlight pipeline deleted from screen+VM. LGPL-2.1: binary dependency only; About attribution + `assets/licenses/SORA_EDITOR_LGPL.txt` shipped; **owner's explicit LGPL acceptance + APK-delta re-measure still gate the merge**. Known regressions recorded: diagnostics tap-popup (surface died), completion popup bottom-anchored. Host tests: `CodeCThemeMapTest` ×5, `CodeCLanguageLogicTest` ×6. No PR/merge without the owner's command.
