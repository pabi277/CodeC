...
**Last updated:** 2026-09-05 · **Head: Phase 29 (VS Code colour / TextMate) 🚧 IMPLEMENTED, CI GREEN — APK-budget deviation pending owner verdict** (owner: "Start phase 29") — all three parts (29.1 `language-textmate` core + themes, 29.2 language parity via the `LanguageType` split, 29.3 regex retirement) built, **CI green** (runs 8e59d47 + trim commit; device round 1's open-file CME crash fixed — sora theme dispatch vs analyzer construction race — registry-monitor + locked-createLanguage + main-thread switch, regression-tested): sora's TextMate engine (same 0.24.6 BOM) powers the editor analyzer with 24 MIT grammar assets + Dark+/Monokai/Dracula/GitHub-Dark themes (default **VS Code Dark+**), lazy grammar loading + background warm-up, host tests incl. the analyzer-swap law (Robolectric against the real assets). **Two build incidents fixed on the way (PART_29_1 §4.5):** Dark+ shipped as `dark-plus.json` while the code asked for `vscode-dark-plus.json` (silent degrade to the previous theme — found by the instrumented test); a `gh run watch` exit code was misread as green during a token blip. **APK delta MEASURED +2.10 MiB (22.0 → 24.2 MB) — OVER the +1.5 MiB budget by ~0.6 MiB: the weight is the required engine chain (joni+jcodings+gson+tm4e ≈ 1.95 MB; all 28 grammar/theme assets ≈ 250 KB compressed; the plan's "defer Go/Rust" remedy targets ~12 KB and cannot close it). Owner verdict needed at the device round / merge gate.** Gate: owner device round (`TROUBLESHOOTING.md` §12; keystroke p95 ≤ 16.7 ms) — records in [`chat-phase29/`](chat-phase29/README.md). **No PR/merge until the owner's command.** Remaining queue: 30 snippets/Emmet, 31 LSP Packages, 32 phone canvas, 33 first-hour (all 📋 PLANNED, docs only — [`chat-phase30/`](chat-phase30/README.md) … [`chat-phase33/`](chat-phase33/README.md); research: [`OSS_REPLACEMENT_RESEARCH.md`](OSS_REPLACEMENT_RESEARCH.md), [`PHONE_UX_ANALYSIS.md`](PHONE_UX_ANALYSIS.md)). 28.3/28.4 remain the Keys remainder.
