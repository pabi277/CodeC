...
**Last updated:** 2026-09-04 · **Current head: Phase 25.1 (editor-core bench spike) IMPLEMENTED on `arena/01a06b20-codec`, base `5ebbc6e` (= `main` after PR #48)** — owner's "Start Phase 25". The throwaway `:bench` module (separate APK `com.codeci.bench`, artifact `CodeC-Bench`) hosts the three candidates (C-now mirror / C-sora 0.24.6 binary dep / C-compose2 visible-window sketch) + identical-input FrameMetrics harness + committed corpus + host tests. **CI green `33849153135`** (app assemble + tests + lint; bench assembleRelease + unit tests; artifacts `CodeC-IDE` + `CodeC-Bench`; four for-cause red rounds fixed — see PART_25_1 §4.4). **Device round REQUIRED: the owner installs `CodeC-Bench` from the green run's artifacts, runs the scenarios, pastes the Copy-all export — then the agent fills `docs/EDITOR_MOBILE_RESEARCH.md` §3.1 and states the gate verdict (25.2 / 25.3 / stay on C-now) in writing.** No PR/merge until the owner's explicit command. Record: `docs/chat-phase25/PART_25_1_SPIKE_BENCH.md` §4; JOURNEY §34.
