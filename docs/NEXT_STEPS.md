...
**In progress (2026-09-01, `arena/01a05b6c-codec`):** owner-reported git bugs — a new branch was created locally only and never reached GitHub, and a commit on a never-published branch looked pushed. Fix: publish NEW branches on creation, push by explicit branch name, and a Projects-card amber `↑` badge for branches not on the remote yet. Record: [`chat-phase15/PART_17_SOURCE_CONTROL.md`](chat-phase15/PART_17_SOURCE_CONTROL.md) §6.3. CI + owner device pass pending.
