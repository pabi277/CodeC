...
**Last updated:** 2026-09-03 · **State (head):** **Phase 21 (D.1 registry + D.2 auto-install gate) 🚧 IMPLEMENTED & CI-pending on `arena/01a064e0-codec`** — `LanguageRunProfile`/`LanguageRegistry`/`LanguageRunPlanner`/`LanguageToolProbe` (all Android-free), `runActiveFile` dispatches generically for 12 languages, `pkg install -y <pkg>` gate streams into the Output Panel and auto-continues the run, and every `TerminalHandoff` command builder now emits `gcc`/`g++`/the right interpreter instead of the TCC `cc` shim (+33 new host tests). **Device round 1 (2026-09-03) FAILED at the install gate → fixed (D17 `requiredPackage` gcc→clang: there is no `gcc` package, the `clang` subpackage ships the gcc symlinks; D18 Go/Rust marked not-in-repo instead of a doomed install; D19 gate now runs `pkg update &&` first). **Phase 21 is ✅ COMPLETE and awaiting the owner's merge command.** D.3 device acceptance PASSED (owner: "Pass"); **D.4 (Remove TCC) CANCELLED by owner** — TCC is the DEFAULT C compiler, the Settings → Compiler Engine picker is deleted (D22) and `.c` now compiles with the built-in `cc` frontend with **no install gate** while `.cpp` still installs clang (D23). Device round 2 had also FAILED → fixed (D20 server presets + custom project.json commands never reached the gate — `python3 app.py` died with exit 127 and no prompt; new `toolchainForCommands` gates raw command strings by their leading program. D21 the `c-microservice` preset still compiled with `cc`).  Record: `docs/chat-phase21/PART_21_IMPLEMENTATION.md`. **Previous head:** **Phase 20.1 ✅ COMPLETE, DEVICE-VERIFIED & MERGED to `main` by owner command (2026-09-03, from `arena/01a05cb9-codec`)** — round-4 toolchain roots (`libllvm`+clang/gcc symlinks, nodejs, npm, php trimmed, ruby, lua54) published to the dev channel and live: final salvage run `33669069048` green (~3h04m) with the D15 (`cc` never created → app's tcc frontend restored after app restart) and D16 (`lua -v` → Lua 5.4.8) fixes device-verified 6/6; bootstrap release `userland-v2-dev` refreshed via `33669089783`. Full record: `docs/chat-phase20/PART_20_1_TOOLCHAINS.md` (build-round log entries 1–12, decisions D1–D16). **Next: Phases 20.2–24 remain PLANNED and fully spec'd — owner starts a phase by saying "Start Phase 21" (or 22/23/24) in chat.** Docs: `docs/chat-phase20/` through `docs/chat-phase24/` + `docs/RESEARCH_NEXT_PHASES.md`. Recommended order: 21 → 22 → 23 → 24 (21 and 22 can run in parallel).
