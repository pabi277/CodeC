...
**Last updated:** 2026-09-08 · **Head: Phase 31 (IntelliSense as Packages) 🚧 31.1–31.6 WIRED on `arena/01a08023-codec` (owner: remaining work was \"wire the editor\")** — hand-rolled LSP stdio JSON-RPC (sora `editor-lsp` AAR stays gated: minSdk 26 vs CodeC 24). Chip strip + ghost merge LSP on `EditorViewModel.refreshCompletionItems` (debounced, Default dispatcher). Device recipe: install clangd → type in `.c` → a member snippets cannot know appears in the CHIP STRIP. Records: [`chat-phase31/PART_31_5_STDIO_WIRE.md`](chat-phase31/PART_31_5_STDIO_WIRE.md). **No PR/merge without the owner's command.** Remaining: 32 phone canvas, 33 first-hour UX. 28.3/28.4 remain the Keys remainder.
