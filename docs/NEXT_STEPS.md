**Last updated:** 2026-09-10 · **Head: Phase 39 (Outputs are temporary, never in your repo) 🔧 IMPLEMENTED on `arena/01a08a0a-codec` — CI ✅ GREEN (`Build APK` `34448677743` on tip `2d9002d`, 5 m 47 s); device exits 1–6 owner-passed; **git branch-tracking UX fix** (sticky unpublished banner + push always names checked-out branch) landed at `aacbd02` — CI ✅ GREEN (`Build APK` `34455576225`, 5 m 15 s). Retest on device.** Owner: "Start Phase 39". What shipped: **39.1** `RunArtifacts` routes CodeC-invented outputs to `filesDir/CodeC/temp/runs/<stamp>/` (user `-o bin/menu` stays put); `TempGc` (age / keepNewest=8 / 128 MiB / busy stamps; walk confined to `runs/`); `LiveRunStamps`; `CompilerService` new layout; `PYTHONPYCACHEPREFIX`; Settings → Storage temporary-files row + Clear; cold-start GC in `MainActivity`. **39.2** `RepoHygiene` ~56 patterns (C/C++, Python, Node/JS/TS, Java/Kotlin, Lua, OS junk, `.codec/` + `.codec.json`) from `github/gitignore` names (CC0; `assets/licenses/GITHUB_GITIGNORE_CC0.txt`); `BuildArtifactIgnore`/`PythonCacheIgnore` folded to thin delegates and deleted; enforcement inside `GitManager.stageAll` (ensure → untrack → `git add -A`; rm-cached failure aborts); COMMIT & PUSH shows "Removed N…" + a "Will commit N file(s)" preview; user `.gitignore` (incl. `!a.out`) always wins. Host tests: `RunArtifactsTest`, `TempGcTest`, `TempGcAndRunInteropTest`, `RepoHygieneTest`, `CommitPreviewTest`, `StageAllHygieneTest` (+ `PythonCacheIgnoreTest` / `SettingsAuditTest` updated for 43→45 Storage rows). **Next action: device round** (phase exit conditions 1–6 on a phone + a real push), then CI green + owner "All device test pass" / merge command. Remaining queue after 39: 40 → 41 → 42 → 43 (39.1 before 43.2 is the one ordering rule — now satisfied). Records: [`chat-phase39/`](chat-phase39/README.md), JOURNEY §55.
