**Last updated:** 2026-09-13 · **Head: Phases 44 + 45 ✅ COMPLETE & MERGED to `main` via [PR #77](https://github.com/pabi277/CodeC/pull/77) on the owner's command (*"Merge it"*) — merge commit `a475fe4`, 21 commits, CI ✅ GREEN four times over (branch `34719753700` tip `8c3c10d`; docs `34720490002` tip `935fe41`; PR `34721374151` on the merge commit `6b5ba21`, check `build` pass 7m52s; **`main` post-merge `34721779445` on `a475fe4`**), zero error annotations, release APK **6,675,150 B** on `main` (whole arc **+23,468 B / +0.35%** over Phase 44 round 4), debug 25,693,528 B, v1.3.17, **199 host cases** run on real Gradle/JUnit/Robolectric in every one of those runs. What landed: **Phase 44** — the one-time install is visible on every tab and **C is never gated** (44.1), and a kill cannot half-finish it (44.2: `SetupLedger`, `resumePlan`, the boot repair, the marker moved after the swap, `userlandUsable` reading the real `bin/pkg`) — plus its device-round-1 corrections (a marker is not a prefix · the bar was a wall in exactly the state it was written for · `restoreState = true` restores a sub-stack, not a tab); **Phase 45** — five vocabulary-pinned slides (45.1) and one ten-beat tour with no skip anywhere mid-tour where a single tap performs the highlighted control's own click and then advances (45.2), plus the chrome lock across rounds 4-6: a per-option pause with one sweet sentence that says what is happening, why and where to watch it, **on from the first frame** of a fresh install (round 5) and **bounded by the setup** so it can never outlive the work it protected (round 6 — a settled stage always reopens the app, and a shell coming alive re-reads the disk). `main` had moved while the branch was open (PR #76 recorded the plan's merge in the same three docs files), so the PR read as conflicting: resolved by merging main into the session branch (`6b5ba21`) and keeping **both** truths verbatim, with no entry renumbered. **THE DEVICE ROUNDS ARE STILL NOT RUN — a merge is not a device pass, and neither phase may be described as device-tested.** Next action: the owner's device rounds on the merged build — [`chat-phase45/DEVICE_ROUND.md`](chat-phase45/DEVICE_ROUND.md) **G1-G41** (G41 is round 6's release row and needs a build that has the fix; start from Settings → About → **Reset tips**) and [`chat-phase44/DEVICE_ROUND.md`](chat-phase44/DEVICE_ROUND.md) **R1-R8**, then **D1-D12** on a fresh install — then `Start Phase 46` (Projects, not folders).** Queue after 45: 46 → 47 → 48 → 49 → 50. Records: [`chat-phase44/PART_44_1_VISIBLE_SETUP.md`](chat-phase44/PART_44_1_VISIBLE_SETUP.md), [`chat-phase45/`](chat-phase45/README.md), TROUBLESHOOTING §32-§40, JOURNEY §60-§66 + **§66a (the merge)**, rule.md §9, prompt.md, PHASE44_50_ROADMAP.
