...
|---|---|---|
| 40 Git UX | **Eclipse JGit** (pure Java, EDL/BSD) | ❌ not adopted — reasons in §1 |
| 40 Git UX | `git` CLI from the CodeC userland (already in the app) | ✅ keep, and *gate* it (readiness) |
| 40 Publish | GitHub REST `POST /user/repos` | ✅ one HTTPS call, no new dependency |
| 43 File system | `androidx.documentfile` (`DocumentFile`/`TreeDocumentFile`, Apache-2.0) | ⚠ reference + optional helper; not the walker |
| 43 File system | `MANAGE_EXTERNAL_STORAGE` — **already declared + offered by CodeC** (`AndroidManifest.xml:12`; three `isExternalStorageManager()` call sites) | ⚠ keep as-is, never make it load-bearing; audited in 42.3 |
| 43 File system | Rclone, a jsoup-style "own picker", SAF-as-the-only-path | ❌ rejected |
| 39 Ignore rules | **`github/gitignore` templates, CC0-1.0** | ✅ vendor the **patterns** |
| 39 Temp outputs | git's own `.git/info/exclude` + `git rm --cached` | ✅ already the house method; extend it |
...
| 38 Icon | AOSP adaptive-icon spec (108/72/66 dp, monochrome layer) | ✅ follow it exactly |
| 38 Icon tooling | `sharp` (BSD-3, npm) · `@resvg/resvg-js` (MPL-2.0) | ✅ build-time only, outputs committed |
| 42 Distribution | **GitHub Releases** (workflow already attaches APKs on `release` events) | ✅ make it the channel |
| 42 Size | R8 (`minifyEnabled` + `shrinkResources`) + ABI splits | ✅ enable, measure |

---

## 1. Phase 40 — Git on Android: what exists, and why we stay with the CLI
...
into 40.3: (a) a fine-grained token scoped to *specific* repositories cannot
...
## 2. Phase 43 — SAF/`DocumentFile`, and the crash class we must kill
...
Three research findings become requirements in 43.1:
...
- **Some picks are not directories.** Picking *Downloads* from the picker's
  ...
  resulting `StackOverflowError` is a **crash**. 43.1's pure policy must
...
- **Do not "improve" this by *leaning on* a permission — and note the app
  ...
  has to work for anybody who declines), and 42.3 audits the declared list for
  ...
  same law `FileManager`'s comment documents and the reason 43.2 mirrors a
  *running* copy inside app storage.

---

## 3. Phase 39 — the ignore rules: vendor patterns, not files
...
and 39.2 takes **the pattern names only** — one curated list per language
...
- `git rm --cached` for what an earlier commit already carried, which is the
  ...
  one call site — 39.2 moves it to the choke point `stageAll`);
...
ignore files; Termux/CodeC's own compiler services use a temp dir. 39.1
...
`cc … -o bin/menu`, which stays in the project and is covered by 39.2.
...
## 5. Phase 38 — the icon: AOSP rules, not taste
...
- **`monochrome` is effectively mandatory now**: Android 13 tints it for themed
  ...
  but it is not a monochrome design; 38.1 supplies a real one.
...
roadmap, not in v2.6.2) [npmx.dev/@resvg/resvg-js]. 38.1 therefore commits
...
## 6. Phase 42 — distribution: what the ecosystem actually allows
...
- **The channel is GitHub Releases, and the machinery exists.** The workflow's
  ...
  **"Latest"** — so the in-app updater's `latest` currently points at a
  ...
  version or digest check). 42.1 is mostly *making what we built honest*.
...
- **R8 is the cheap win.** `isMinifyEnabled = false` today; enabling
  ...
  study for the option set]. 42.2 makes this measurement-led (before/after
...
- **ABI weight.** `ndk { abiFilters += [arm64-v8a, armeabi-v7a, x86_64, x86] }`
  ...
  (AOSP-supported, no dependency) is how 42.2 cuts the phone download roughly
...
- **Backup is currently mis-declared.** `allowBackup="true"` with
  ...
  is to scope these files or disable backup; 42.3 does the latter with a
...
- **Crash-loop safety net.** Nothing existing to take; the app already keeps a
  ...
  so 42.3 is a pure "how many consecutive launch crashes before we open in safe
