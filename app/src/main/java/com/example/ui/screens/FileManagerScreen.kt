...
import com.example.ui.viewmodels.FileManagerViewModel
import java.io.File
            ...
            confirmButton = {
                TextButton(onClick = {
                    if (fileName.isNotBlank()) {
                        ...
                        if (result.isFailure) {
                            Toast.makeText(context, result.exceptionOrNull()?.message ?: "Failed to create file", Toast.LENGTH_SHORT).show()
                        } else {
                            onFileSelected(if (fileName.endsWith(".c")) fileName else "$fileName.c")
            ...
            confirmButton = {
                TextButton(onClick = {
                    if (newName.isNotBlank() && newName != file.name) {
                        if (!viewModel.renameFile(context, file, newName)) {
...
fun FileItem(
    ...
    val settingsManager = remember { com.example.ui.settings.SettingsManager(context) }
                ...
                DropdownMenuItem(
                    ...
                    onClick = {
                        ...
                        try {
                            ...
                            Toast.makeText(context, "Cannot share directly without FileProvider", Toast.LENGTH_SHORT).show()
                        }
                    }
                )
            }
